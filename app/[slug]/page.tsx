...
export function generateMetadata({ params }: PageProps): Metadata {
  ...
  return {
    title: `Schedule a Meeting with ${member.name} | Business Automations Expert`,
    description: `Book a consultation with ${member.name}, ${member.role} at Business Automations Expert.`,
  }
}

export default function TeamMemberSchedulePage({ params }: PageProps) {
  const member = team.find((m) => m.slug === params.slug)
    ...
    <main className="relative z-10 px-4 sm:px-6 lg:px-8 pt-10 pb-20">
      <div className="w-[90%] mx-auto text-center mb-8">
        <h1 className="text-3xl sm:text-4xl font-bold text-untitled-ui-white">
          ...
          <span className="text-primary-accent">{member.name}</span>
        </h1>
        <p className="mt-3 text-base sm:text-lg text-untitled-ui-white/70">
          Book time with our {member.role} to discuss your automation needs.
        </p>
      </div>
      ...
      <div className="w-[90%] mx-auto" style={{ height: "800px" }}>
        <iframe
          ...
          scrolling="yes"
          style={{ border: 0 }}
          title={`Schedule a meeting with ${member.name}`}
        />
      </div>
    </main>
  )
